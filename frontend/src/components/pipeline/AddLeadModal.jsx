/**
 * AddLeadModal — modal form to create a new lead + company
 * Upserts the company on conflict(name, entity), then inserts the lead at stage=new_lead
 */
...
export default function AddLeadModal({ onClose, onCreated }) {
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true)
    try {
      // 1 — Upsert company (unique on name+entity)
      ...
      toast({ type: 'success', message: 'Lead added' })
      onCreated?.(lead)
      onClose()
    } catch (err) {
      console.error(err)
      toast({ type: 'error', message: err.message ?? 'Failed to add lead' })
    } finally {
      setSaving(false)
    }
  }
        ...
        <form onSubmit={handleSubmit} style={{ padding: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px', gap: '10px', marginBottom: '14px' }}>
            <div>
              ...
              <input
                ...
                placeholder="e.g. Steel Masters Co."
                required
              />
            </div>
            <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '18px' }}>
            <div>
              ...
              <input
                className="crm-input"
                value={form.estimated_gmv_month}
                onChange={e => set('estimated_gmv_month', e.target.value)}
                placeholder="50000"
                type="number"
                min="0"
              />
            ...
            <div>
              ...
              <input
                className="crm-input"
                value={form.deal_success_rate}
                onChange={e => set('deal_success_rate', e.target.value)}
                placeholder="50"
                type="number"
                min="0"
                max="100"
